...
# --- Optional: style the primary button to appear inverted (black/white) ---
st.markdown("""
<style>
/* Make primary buttons high-contrast */
div.stButton > button[kind="primary"]{
    background-color:#000000 !important;
    color:#FFFFFF !important;
    border-color:#000000 !important;
}
/* Slightly tighten form spacing */
section[data-testid="stSidebar"] div.block-container {padding-top: 0.6rem;}
...
# ---------------------- After Calculate: pull values ----------------------
p = st.session_state.params

# Sidebar quick edits (optional)
...
cycle_choice = st.sidebar.radio(
    "Reefer cycle", ["Continuous", "Start-Stop", "Reefer OFF"],
    index={"Continuous": 0, "Start-Stop": 1, "NoReeferStationary": 2}.get(p["ReeferCycleInit"], 0)
)
p["ReeferCycleInit"] = "NoReeferStationary" if cycle_choice == "Reefer OFF" else cycle_choice
st.session_state.params = p
...
tp_hours = np.arange(25)                 # 0..24
h24      = np.arange(1440)/60.0          # minutes -> hours
...
figLegend.legend(
    handles=legend_items, loc='upper center', ncol=len(legend_items), frameon=False,
    prop={'size': 6}, handlelength=1, handletextpad=0.6, borderpad=0.2, labelspacing=0.4, columnspacing=1.2
)
...
axW.fill_between(h24, P_norm_W,   step='pre', color=colW_dumb,  alpha=0.30)
axW.fill_between(h24, P_smart_W,  step='pre', color=colW_smart, alpha=0.35)
axW.set_ylabel('Power (kW)'); axW.grid(True, linestyle=':', alpha=0.6)
leftYL = axW.get_ylim()
if leftYL[1]-leftYL[0] <= 0:
    axW.set_ylim(0, 1); leftYL = axW.get_ylim()
...
axW.step(tp_hours, yPriceW, where='post', color=colPrLn, lw=1.0)
ax2 = axW.twinx()
ax2.plot(h24, SOC_norm_W, '-', color=colSoC_d, lw=1.1)
ax2.plot(h24, SOC_smart_W,'-', color=colSoC_s, lw=1.1)
ax2.set_ylim(0, 100); ax2.set_ylabel('SoC (%)')
axW.set_xlim(0, 24); axW.set_xticks(np.arange(0, 25, 1)); axW.set_xlabel('Time of the day')
yTxtW = yBase + ((eW24 - eMinW)/(eMaxW-eMinW) if eMaxW!=eMinW else np.zeros(24))*(yTop-yBase)
for h in range(24):
    axW.text(hMid_hours[h], yTxtW[h], f"{eW24[h]:.2f}", color=colPrTx, fontsize=9, fontweight='bold',
             ha='center', va='bottom')
st.pyplot(figW, use_container_width=True)

# --- Summer ---
figS, axS = plt.subplots(figsize=(12, 3))
axS.set_zorder(2); ax2S = axS.twinx(); ax2S.set_zorder(1); axS.patch.set_alpha(0)
...
axS.set_ylabel('Power (kW)'); axS.grid(True, linestyle=':', alpha=0.6)
leftYL = axS.get_ylim()
if leftYL[1]-leftYL[0] <= 0:
    axS.set_ylim(0, 1); leftYL = axS.get_ylim()
...
axS.set_xlim(0, 24); axS.set_xticks(np.arange(0, 25, 1)); axS.set_xlabel('Time of the day')
yTxtS = yBaseS + ((eS24 - eMinS)/(eMaxS - eMinS) if eMaxS != eMinS else np.zeros(24)) * (yTopS - yBaseS)
for h in range(24):
    axS.text(hMid_hours[h], yTxtS[h], f"{eS24[h]:.2f}",
             color=colPrTx, fontsize=9, fontweight='bold', ha='center', va='bottom', zorder=10, clip_on=False)
...
10. Fixed electricity price: €{rp.FixedPrice_EUR_per_kWh:.2f} per kWh.
11. Diesel price: €{rp.DieselPrice_EUR_per_L:.2f}/L; DG efficiency: {int(100*rp.Genset_efficiency_frac)}%;
    Energy density: {rp.Diesel_kWh_per_L:.1f} kWh/L.
"""
    )
